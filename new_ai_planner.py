...
def generate_ml_study_plan(subjects_data):
    ...
    for sub in subjects_data:
        # ✅ Modified line: handle both 'name' and 'subject' keys
        name = sub.get('name') or sub.get('subject') or 'Unknown'
        
        score = sub.get('score', 0)
        class_avg = sub.get('classAvg', 0)
        max_score = sub.get('maxScore', 0)
        
        # Optional support for new AI planner input
        marks = sub.get('marks', 0)
        credits = sub.get('credits', 'N/A')

        # Calculate priority indicators (fallback-safe)
        below_avg = (class_avg - score) if class_avg and score else 0
        gap_from_max = (max_score - score) if max_score and score else 0
        
        info_str = f"""
- Subject: {name}
  Student Score: {score or marks}%
